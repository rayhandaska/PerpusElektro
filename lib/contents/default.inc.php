...
if (isset($_GET['search']) && !empty($_GET['search'])) {
  ...
  if ($keywords && !preg_match('@[a-z0-9_.]+=[^=]+\s+@i', $keywords.' ')) {
    $criteria = 'title='.$keywords.' OR author='.$keywords.' OR subject='.$keywords. 'OR publish_year='.$keywords;
  ...
  $is_adv = isset($_GET['title']) || isset($_GET['author']) || isset($_GET['isbn'])
    || isset($_GET['publish_year']) || isset($_GET['subject']) || isset($_GET['location'])
  ...
  if ($is_adv) {
    ...
    if ($title || $author || $subject || $isbn || $gmd || $colltype || $location || $publisher || $callnumber || $publish_year !== '') {
      $criteria = '';
      ...
      if ($publish_year) { $criteria .= ' publish_year="'.$publish_year.'"'; }
      //print_r($criteria);
      //die();
  ...
  if ($is_adv) {
    ...
    if ($publish_year) { $search_result_info .= __('Tahun Masuk Buku').' : <strong><cite>'.$publish_year.'</cite></strong>, '; }
    // strip last comma

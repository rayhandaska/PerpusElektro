...
if (isset($_GET['action']) AND $_GET['action'] == 'print') {
  ...
  $html_str .= '<html><head><title>Item Barcode & Label Print Result By SLiMS Community - Mudified by Adhi SLiMS</title>'."\n";
  ...
  $html_str .= '.labelStyle1 { width: '.$sysconf['print']['barcode']['barcode_box_width'].'cm; height: '.$sysconf['print']['barcode']['barcode_box_height'].'cm; text-align: center; margin: '.'0'.'cm; }'."\n";
  $html_str .= '.barcode_rotate { position: relative; width:1.4cm; -moz-transform: rotate(-90deg); -webkit-transform: rotate(-90deg); -o-transform: rotate(-90deg); -ms-transform: rotate(-90deg); transform: rotate(-90deg);}'."\n";
  $html_str .= '</style>'."\n";
  $html_str .= '</head>'."\n";
  $html_str .= '<body>'."\n";
            ...
            if ($sysconf['print']['barcode']['barcode_include_header_text']) {}
				  ...
				  $html_str .= '<tr>';
				    if ($sysconf['print']['barcode']['barcode_cut_title']) {         			
				  	$html_str .= '<td rowspan="2" style="border-right: '.$sysconf['print']['barcode']['barcode_border_size'].'px solid #000000; padding-left:0cm; padding-top:3cm;"><div class="barcode_rotate">'.'<div style="font-size: 6pt; width: 4.5cm;">'.substr($barcode[0], 0, $sysconf['print']['barcode']['barcode_cut_title']).'...</div>'.'<img src="'.SWB.IMG.'/barcodes/'.str_replace(array(' '), '_', $barcode[1]).'.png" style="width: 4.5cm; height: 1.1cm; border="0"/>';
					} else { $html_str .= $barcode[0]; }
				    $html_str .= '</td>';
				  $html_str .='<td style="background-color:'.$warna.'; border-bottom:3px solid #000000;"><div style="font-size: 8pt; font-weight:bold;">'.($sysconf['library_name']).'</div><div style="font-size: 10pt; font-weight:bold;">'.($sysconf['library_subname']).'</div>';
				  $html_str .= '</td>';
			       if ($sysconf['print']['barcode']['barcode_cut_title']) {         			
				  	$html_str .= '<td rowspan="2" style="border-left: '.$sysconf['print']['barcode']['barcode_border_size'].'px solid #000000; padding-left:0cm; padding-top:3cm;"><div class="barcode_rotate">'.'<div style="font-size: 6pt; width: 4.5cm;">'.substr($barcode[0], 0, $sysconf['print']['barcode']['barcode_cut_title']).'...</div>'.'<img src="'.SWB.IMG.'/barcodes/'.str_replace(array(' '), '_', $barcode[1]).'.png" style="width: 4.5cm; height: 1.1cm; border="0"/>';
					} else { $html_str .= $barcode[0]; }
				  $html_str .= '</td></tr>';
				  $html_str .= '<tr><td>';
					$sliced_call_number = explode(' ', $barcode[2], 5);
	              	foreach ($sliced_call_number as $slice_call_number_label) {				  
				  	$html_str .= '<div style="font-size: 12pt; font-weight:bold; text-align:center; font:Arial;">'.$slice_call_number_label.'</div>'; }
				  $html_str .= '</td>';
				  $html_str .= '</tr>';
				  $html_str .= '</table>';
			$html_str .= '</div>';
...
<div class="menuBoxInner printIcon">
  ...
  <div class="sub_section">
	  <div class="btn-group">
      <a target="blindSubmit" href="<?php echo MWB; ?>bibliography/barcode_generator.php?action=clear" class="notAJAX btn btn-default"><i class="glyphicon glyphicon-trash"></i>&nbsp;<?php echo __('Clear Print Queue'); ?></a>
      <a target="blindSubmit" href="<?php echo MWB; ?>bibliography/barcode_generator.php?action=print" class="notAJAX btn btn-default"><i class="glyphicon glyphicon-print"></i>&nbsp;<?php echo __('Print Barcodes for Selected Data');?></a>
    ...
    <form name="search" action="<?php echo MWB; ?>bibliography/barcode_generator.php" id="search" method="get" style="display: inline;"><?php echo __('Search'); ?> :
...
$datagrid->column_width = array('25%', '40%');
